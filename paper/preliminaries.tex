...
\newcommand{\dReturn}{\textbf{e}}
The set of well-matched words over a pushwdown alphabet $\vAlph$ is the least set containing $\epsilon$ 
such that
(1)~if $w$ is well-matched, $c$ is a call letter and $r$ is a return letter, then $ c w r$ is well-matched, 
(2)~if $w$ is well-matched and $l$ is a local letter, then $lw$ and $wl$ are well-matched, and
(3)~if $w_1, w_2$ are well-matched, then the concatenation $w_1 w_2$ is well-matched.
...
A \VPA $(\vAlph, \stackAlphabet, Q, Q_0, \delta, F)$  is deterministic (called $\DVPA$), if $|Q_0| = 1$ and 
