...
where the learning algorithm is provided with \emph{advice} expressing prior knowledge regarding the target language.
Given advice, 
...
By using the advice to answer queries deductively, this framework bridges two synthesis paradigms:
...
Furthermore, a simple property ``all content between the opening comment tag and the corresponding closing tag can be discarded''
is not expressible by simple rewrite rules, as it involves matching of the opening and closing tags.
We address this issue by introducing rewrite rules with variables, which can be substituted with \emph{well-matched} words.
...
from regular languages to \VPLs and Deterministic Visibly Pushdown Automata (DVPAs) is technically demanding. 
