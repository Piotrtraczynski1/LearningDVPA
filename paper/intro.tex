...
rules out polynomial-time active learning algorithms for \DVPA (unless \P=\NP)
...
consider a different automata model for language representation, called k-SEVPA~\cite{DBLP:phd/dnb/Isberner15, ValidatingXMLwithVPA,DBLP:journals/pacmpl/JiaT24}, 
...
including inferring specifications for document validation from JSON schemas, 
...
\Paragraph{Learning with advice}
    We consider an active learning framework augmented with advice, 
...
\begin{enumerate}
    \item 
    We identify string rewriting rules that are compatible with the structural properties of pushdown languages.
    Furthermore, we extend these rules from word pairs to rules with variables in order to capture basic pushdown language properties.
...
(e.g., oracles that answer additional types of queries, or assuming the set of non-terminals is known), or altering the mode of learning
Improving the performance of active learning algorithms by restricting learned languages to particular automaton subclasses is an active research direction for Mealy machines~\cite{graybox-learning, DBLP:conf/dlt/BerthonBPR21, DBLP:conf/fossacs/LabbafGHM23, DBLP:journals/corr/abs-2405-08647, ProductAutomata} and tree automata~\cite{DBLP:conf/fossacs/HeerdtKR021, BjorklundBE17}. 
