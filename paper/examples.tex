...
The probabilistic equivalence test is significantly faster than the exact one.
...
a \DVPA consistent with \emph{well-matched commutativity}, we generate two random \DVPA over disjoint alphabets, take the product of these automata, where undefined transitions are defined as self-loops. 
...
We observe that, for well-matched cancellation property ($c X r \to \epsilon$), increasing
...
\Paragraph{Summary}
The presented results show a reduction of approximately $20\%$ in the number of queries for (well-matched) commutativity and idempotency,
...
Its cost is negligible with exact equivalence check but becomes noticeable with the probabilistic one.
