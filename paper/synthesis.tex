...
we study how advice provided via an SRS reduces the number of equivalence queries necessary to learn an unknown language.
...
high computational complexity and the lack of a clear objective (what is the right notion of a ``relevant rule''?). 
...
The latter assumption is to ensure that there are well-matched words other than the empty word.
...
In order to achieve that, we give  sufficient conditions for pairs of words to form a rewrite rule consistent with $\lang(\aut)$.
...
transforming states into states, which corresponds to the syntactic congruence.
Formally, for a state $q \in Q$ we define $f_w(q) = q'$ if and only if for some stack content $u$, we have $(q,u) \trans{\aut}{a} (q',u)$.
Note that ``some $u$'' can be replaced by ``all $u$'' as the transitions over well-matched words do not depend on the initial stack content. 
...
where $w$ is a well-matched word.
There are $2^\Theta(k)$ well-matched words of the length $k$, and hence among well-matched words of length $\bigO(n \log n)$ 
there is a pair of well-matched words $\lhs, \rhs$ such that $\lhs \to \rhs$ is a ground well-matched rule consistent with $\lang(\aut)$. 
...
the sum of lengths of $w_0, \ldots, w_k$ is bounded by  $2^{c \cdot k\cdot n \cdot \log(n)}$, 
there is a pair of words that forms a rewriting rule consistent with $\lang(\aut)$.

\todo{The paragraph beginning ``The counting argument shows…'' shifts from the upper-bound argument to a lower-bound discussion mid-paragraph, which feels abrupt. Consider splitting it into two paragraphs: one concluding the upper-bound result, and a new one opening the complexity discussion}
...
polynomial time. Clearly, not all consistent rules are of polynomial length even if some form of nonredundancy. 
Even in the \DFA case, we can construct an automaton $\aut$ such that there are words $w$ such that 
$w \to \epsilon$ is a rule consistent with $\lang(\aut)$, but every such $w$ is of exponential length. 
It suffices to define a DFA such that the transition over any letter is the same permutation of an exponential order.
...
The main difficulty of the proof is to ensure that if the formula is unsatisfiable, 
...
\todo{The paragraph on Bounded rewriting to single letter introduces a new but closely related problem. Its connection to the minimal collision problem is not made explicit — a single sentence explaining the relationship (or distinguishing the two) would aid readability}
...
given a \DFA $\aut$ over $\Sigma$,  a letter $a \in \Sigma$, and a bound $k>0$, 
...
asks, given $m,k>0$ and \DFA $\aut_1, \ldots, \aut_m$, whether 
there is $w$ such that $|w| \leq k$ and 
$w \in \lang(\aut_i)$ for every $i$.
