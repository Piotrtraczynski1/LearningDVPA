...
Formally, for a state $q \in Q$ we define $f_w(q) = q'$ if and only if for some stack content $u$, we have $(q,u) \trans{\aut}{w} (q',u)$.
...
Now, a natural question is whether non-determinism is necessary.  
...
\Paragraph{Rules with one side fixed}
While minimal-length rules are polynomial, certain consistent rules can be exponential in length—even when restricted to ``nonredundant'' rules. 
Even in the \DFA case, consider a family of \DFA  $\aut_n$ with $|Q| = n$ where for every $a \in \Sigma$, the function $f_a$ is the same permutation of states of an exponential order.
For example, a permutation composed of disjoint cycles of prime orders $p_1, \ldots, p_k$ has the order of $p_1 \cdot \ldots \cdot p_k$.
In this setting, there exist non-empty words $w$ such that $w \to \epsilon$ is consistent with $\lang(\aut_n)$, but every such word $w$ has a length at least exponential in $n$.
Furthermore, if one side of the rule is fixed, i.e., for a fixed $v$ and given $K>0$, we ask whether there exists $w$ such that $|w| \leq K$ and $w \to v$ is consistent, the problem becomes \PSPACE-complete, as it corresponds to the \emph{membership}  problem for monoids~\cite{Kozen77}.
