...
matching-preserving rules that are not well-matched (Section~\ref{s:matching-preserving}).
...
We show thus by reduction from the 3-SAT problem. Given a formula $\varaphi$, we rename variables to obtain $\varphi^*$, in which  each variable occurs at most once. 
Then, $\aut$ moves over some instance of $\lhs$ from $q$ to $s_1$ if the variable substitution encodes a truth assignment, where all copies of the same variable have the same value.
Next, $\aut$ moves over some instance of $\rhs$ from $q$ to $s_2$  if  the variable substitution encodes a truth assignment satisfying the formula $\varphi$.
