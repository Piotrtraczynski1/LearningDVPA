...
\Paragraph{Reduction to single-step rewriting}
 ...
 if and only if for all words $x,y$, we have $x$ can be rewritten in a single step to $y$ implies that
 $x \in \in \lang(\aut) \iff y \in \lang(\aut)$.
% $x \lhs y \in \lang(\aut) \iff x \rhs y \in \lang(\aut)$. 
 Indeed, if this property holds for every rule, then 
...
% We show this by reduction from the 3-SAT problem. 
% Given a formula $\varphi$, we rename variables to obtain $\varphi^*$, in which  each variable occurs at most once. 
% Then, variables in rules correspond to variables in $\varphi^*$, and
...
% Next, $\aut$ moves over some instance of $\rhs$ from $q$ to $s_2$  if  the substitution of $\rhs$ 
% encodes a truth assignment satisfying $\varphi^*$.
