...
\Paragraph{Reduction to single-step rewriting}
 Observe that a rule with variables $\lhs \to \rhs$ is consistent with the language of $\aut$ 
 if and only if for all words $x,y$ such that $x$ can be rewritten in a single step to $y$,  it holds that
  $x \in \lang(\aut) \iff y \in \lang(\aut)$.
% $x \lhs y \in \lang(\aut) \iff x \rhs y \in \lang(\aut)$. 
 Indeed, if this property holds for every rule, then  consistency under multi-step rewriting follows by transitivity.
While this observation may seem trivial, it holds due to the assumption that $\aut$ and the rewrite rule are over the same alphabet~\cite{ecai25}.
...
\Paragraph{Ground well-matched rules} 
Consider a well-matched rule $\lhs \to \rhs$ without variables. 
...
Since $\lhs$ is well-matched, the run over $\lhs$ does not depend on the stack content and it does not modify the stack content, and hence
it is equivalent to a transition over a single local letter. 
The \DVPA $\aut_2$ is defined analogously, but $\#$ simulates the word $\rhs$ instead. Again, word $\rhs$ is well-matched. 
To check consistency of  $\lhs \to \rhs$  with $\lang(\aut)$ it suffices to check whether there is a word $w$ such that exactly one of \DVPA $\aut_1, \aut_2$ accepts it.
This is done by a single reachability check on the product \DVPA $\aut_1 \times \aut_2$, a \DVPA simulating two copies of $\aut$ in parallel.
...
The \VPA $\autCon$ is built on the product $\aut \times \aut$, whose stack tracks the contents of both copies of $\aut$.
...
Second, transitions in each copy of $\aut$ must be defined jointly on the product to ensure that both sides of the rule $\lhs \to \rhs$ are instantiated to the same word. 
For example, for a rule $c X r \to X$, there may be a transition in $\aut$ from $q$ to $q_1$ over $c w_1 r$ and from $q$ to $q_2$ over $w_2$, but not with $w_1 = w_2$.
...
\Paragraph{Computing relation $\DeltaRule$}
...
It can be computed iteratively in polynomial time; it amounts to reachability in the product automaton $\aut \times \aut$.
Next, we extend the alphabet $\vAlph$ to $\vAlph_+$ by adding a pair of fresh local letters $x^L$ and $x^R$ for every variable $X$ occurring in $\lhs \to \rhs$.
...
% We reduce from the 3-SAT problem.
% Given a 3-CNF $\varphi$, we rename variables to obtain $\varphi^*$, in which each variable occurs at most once. 
% Then, we define $\lhs \to \rhs$ such that variables in rules correspond to variables in $\varphi^*$, and
% $\aut$ moves over some instance of $\lhs$ from $q$ to $s_1$ if 
% the substitution of $\lhs$ encodes a truth assignment, where all copies of the same variable from $\varphi$ have the same value.
% Next, $\aut$ moves over some instance of $\rhs$ from $q$ to $s_2$  if the  truth assignment satisfies $\varphi^*$.
% These two conditions hold precisely when $\varphi$ is satisfiable.
 %Thus, $(q, s_1, s_2) \in \DeltaRule$ if and only if the above two conditions hold.
 %The latter holds precisely when $\varphi$ is satisfiable.
% We can reduce unsatisfiablity of $\varphi$ to positive (resp. negative) consistency. 
...
The \coNP-hardness result for positive consistency does not extend to consistency.
...
Finally, a state $(q_1, q_2)$ of $\autExt$ is accepting if exactly one of $q_1, q_2$ is accepting in $\aut$. The following condition holds:
\begin{claim}
...
%Consistency of matching-preserving rules with variables reduces to the ground case as in the setting of well-matched rules. 
Consider a matching-preserving rule $\lhs \to \rhs$ with variables.
It suffices to extend the alphabet with local letters $x^L, x^R$ for each variable and then consider a ground rule
obtained from  $\lhs \to \rhs$ by replacing the occurrence of every variable $X$ with $x^L$ in $\lhs$ and with $x^R$ in $\rhs$ whenever it appears.
