...
We discuss separately well-matched rules (Section~\ref{s:well-matched}) and 
matching-reserving rules that are not well-matched (Section~\ref{s:matchchin-preserving}).
In both cases, we consider first rules without variables, and then the extension to rules with vaiables.
While the asymptotic complexity for well-matched and matching-reserving rules is the same,
the algorithms for well-matched rules are conceptually simpler and can be implemented more efficiently.
Before studying paricular types of rules, we make a common observation about deciding complexity.
...
Both \DVPA extend $\aut$ and differ only in behavior on the letter $\#$; in $\aut_1$ for any state $q$, 
...
Therefore, the \DVPA the outcome of $\lhs$ on $\aut$ is equivalent to a transition over a local letter. 
The \DVPA $\aut_2$ is obtained in a similar way, but $\#$ simulates the word $\rhs$, which is also well-matched. 
...
Therefore, we compute a set $\DeltaRule$ of triples $(q, s_1, s_2)$ such that there are words $\alpha, \beta$ satisfying:
...
It is computed iteratively in a similar way to reachability in pushdown automata; it just involves the product automaton $\aut \times \aut$.
...
For a given assignment $\sigma$, we extend the \DVPA $\aut$ to a \VPA $\autPlus$ working over the extended alphabet, where transitions over new letters
defined as follows.
...
Significant improvement the complexity is unlikely as the membership problem for $\DeltaRule$ is \NP-complete in general, and 
...
The key difference is that both sides in well-matched rules neither depend on the stack content nor they modify it, which
is not the case for matching preserving rules.
In consequence, instead of local letters, we extend the alphabet with call and return letters. 
...
and $m$ fresh call letters $\call_1, \ldots, \call_m$. Assume that $k>0$.
At every state of the product, the transition over $\return_1$ simulates the word $v_0^L r_1^L v_1^L$  on the first component of $\autExt$
and it simulates the word $v_0^R r_1^R v_1^R$ on the second component.
...
For new call letter $\call_i$ the transition over $\call_i$ simulates reading the word $c_i^L v_{k+i}^L$ on the first component and $c_i^R v_{i+k}^R$ on the second component.
...
\begin{proposition}
Consistency (resp.,  positive, negative consistency) of a ground matching-preserving rule with the language of a given \DVPA $\aut$ can be decided in polynomial time via a single reachability check on a \DVPA obtained from the product $\aut \times \aut$, and the automaton for a regular expression. 
\end{proposition}
