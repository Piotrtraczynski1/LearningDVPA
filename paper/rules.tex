...
While extending the framework from regular to visibly pushdown languages is conceptually straightforward, 
the two frameworks differ in the class of admissible rewriting rules.
...
The advice supplied to the learning  algorithm is represented via string rewriting systems (SRSs), 
...
$\rew_{+}$ (resp. $\rew_{-}$), which is positively (resp., negatively) consistent with $\targetLang$, 
the task is to construct a \DVPA that recognizes $\lang$. 
...
Therefore, it can be integrated with any framework for active learning of visibly pushdown languages~\cite{DBLP:conf/icgi/BarbotBFHKLN0Y21,DBLP:journals/pacmpl/JiaT24} or \DVPA~\cite{mfcs22} 
provided that the oracle answers membership and equivalence queries.
In particular, the oracle may also answer additianal types of queries as in~\cite{mfcs22}.


\Paragraph{Reducing the number of equivalence queries}
...
we intercept the query and first check whether the advice SRS $\rew$ is consistent (resp., positively consistent or negatively consistent) with
the language of a given candidate automaton $\aut$. 
If it is inconsistent, we conclude that $\lang(\aut)$ differs from the target language with which $\rew$ is consistent (resp., positively/negatively consistent).
...
Rewrite rules should therefore preserve the call--return matching everywhere except within the subword being rewritten.
Intuitively, rewriting one part of the word should not affect other parts of the word, which is trivially true for regular words, 
but not for pushdown words. For instance, by rewriting $c_1 c_2 c_3 r_3 r_2 r_1$ with the rule $c_3 r_3 \to \epsilon r_3 c_3$
...
\begin{definition}
A rewrite rule $\lhs \to \rhs$ \emph{preserves matching} over $\vAlph$ if and only if for all words $w,w' \in \vAlph^*$
...
Matching-preserving rules do not brake the matching between calls and returns that are not in the rewritten fragment of the word.
A prominent subclass of matching preserving rules is the set of \emph{well-matched rules}, which are natural and enjoy more efficient algorithms for checking consistency:

\begin{definition}
A rewrite rule $\lhs \to \rhs$ is well-matched over $\vAlph$ if and only if words $\lhs$ and $\rhs$ are well-matched.
\end{definition}

Every well-matched rule preserves matching. However, not every matching-preserving rule is well-matched: for instance, a rewrite rule $c_1 c_2 r_2 \to c_1$
preserves matching, bit it is not well-matched. Most of our examples are well-matched rules.


While we have restricted admissible rules to those that affect pushdown words locally, 
we now extend them to capture natural properties of pushdown languages. 
...
Unfortunately, this property cannot be expressed by any finite set of string rewriting rules as it requires  exchanging arbitrarily distant letters; for example 
...
For example, a rule $c X r \to X$ generates an infinite set $\{ c r \to \epsilon,c c r r \to \epsilon cr, \ldots \}$.
Finally, an \SRSV $\rewV$ generates an SRS $\sem{\rewV}$ defined as the union of $\sem{\lhs \to \rhs}$  over all rules $\lhs \to \rhs$ from $\rewV$.
We say that an an \SRSV $\rewV$ is consistent (resp., positively/negatively consistent) with a language $\lang$ if and only if 
...
We say that $\lhs \to \rhs$ preserves matching (resp. is well-matched) if and only if the rule obtained by deleting of all variables from $\lhs \to \rhs$ preserves matching (resp. is well-matched).
...
We restrict variable instantiations to well-matched words, since non-well-matched instantiations may violate matching preservation.
Consider the rule $c X r \to X$. Every instance obtained by substituting variables with well-matched words results in a well-matched rule.
In contrast, substituting $X$ with $r c$ yields $c r c r \to r c$, which does not even preserve matching, since $c r c r$ is well-matched, while $r c$ is not.
Furthermore, restricting to well-matched instantiations facilitates deciding consistency with a given \DVPA.
