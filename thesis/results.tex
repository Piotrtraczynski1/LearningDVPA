...
Na podstawie powyższych wykresów można wnioskować, że dominującym czynnikiem wpływającym na liczbę zadawanych zapytań
jest liczba stanów automatu. Zmiany alfabetu miały znacznie mniejszy wpływ na liczbę zadawanych zapytań.
Średnia liczba zapytań $EQ$ jest w przybliżeniu równa liczbie stanów automatu.
Można także zaobserwować podobieństwo w liczbie zadanych zapytań $MQ$ oraz $SC$. Ponadto zaskakującą obserwacją
jest podobieństwo wyników przy rosnącej liczbie symboli typu call oraz local. Oczekiwano, że wzrost liczby
symboli typu call znacząco zwiększy liczbę zapytań w porównaniu z symbolami typu local.
Można wnioskować, że liczba symboli typu return w porównaniu do call oraz local ma największy wpływ
