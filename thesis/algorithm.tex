...
hipoteza będzie bardziej zbliżona automatu docelowego. Procedura ta jest powtarzana, aż do skonstruowania poprawnego automatu. 
...
zbioru $S$. Własność domkniętości nie musi być zachowana z powodu nieokreślonych przejść, jednakże przekierowanie ich od odpowiedniego stanu
...
Jednak podczas testowania algorytmu okazało się, że lemat ten nie jest całkowicie poprawny. Jeśli $s' = \epsilon$ oraz algorytm nie znajdzie
...
ten sam stan ale świadczył o jego osiągalności z innymi symbolem stosowym. Podczas rozważania kontrprzykładu powinniśmy dodać nowy selektor,
który prowadzi do stanu $s$ zostawiając symbol $B$ na szczycie stosu. Jednak nie jest to poprawne rozwiązanie,
ponieważ dwa słowa, które nie są rozróżniale ze względu na aktualny zbiór $C$ mogą prowadzić do różnych stanów a automacie docelowym. Może to skutkować
dodaniem do danej listy selektorów słowa, które świadczy o osiągalności z pewnym symbolem na szycie stosu, który nie jest osiągalny w automacie docelowym.
W konsekwencji prowadziło to do generowania redundantnych stanów w hipotezie.

Następnym rozważanym pomysłem, było utrzymywanie struktury wykluczonych przejść, dla symboli return oraz odwoływanie się do niej,
podczas domykania nieokreślonych przejść w automacie.

Finalnie zastosowane zostało podejście, w którym zamiast utrzymywać wykluczone przejścia, utrzymujemy jedno wymuszone przejście do selektora
równoważnego słowu $va$, selektor taki musi istnieć, ponieważ w przeciwnym przypadku dodalibyśmy nowy selektor w trakcie rozważania tego
kontrprzykładu. Zastosowana modyfikacja wpływa na sposób generowania kolejnych hipotez i w przeprowadzonych testach
pozwalała unikać obserwowanych zapętleń algorytmu. Nie udało się jednak formalnie udowodnić,
że modyfikacja ta w każdym przypadku gwarantuje progres algorytmu.
...
\textbf{Główna ideea} algorytmu polega na wykonaniu następującej procedury.
...
\begin{enumerate}
    ...
    \item Zamienia automat $P'$ na równoważną gramatykę bezkontekstową.

    \item Sprawdza czy utworzona gramatyka bezkontekstowa jest pusta, jeśli nie zwraca słowo $w$ akceptowane przez tą gramatykę.

    \item Jeśli słowo zwrócone przez gramatykę jest puste, nauczyciel odpowiada, że automaty są równoważne, inaczej znajduję
...
aktualnie rozważanego. Rozwiązanie to wciąż nie było wystarczające z powodu na pamięć wykorzystywaną do reprezentowania gramatyki.

\textbf{Ostateczne rozwiązanie}, które zostało zaimplementowane polega na wykonaniu tego algorytmu z całkowitym pominięciem kroku 3, nie wymaga jawnej
konstrukcji gramatyki bezkontekstowej, ale stosuję tę samą idee sprawdzania generatywności nieterminali wprost na automacie.
Algorytm utrzymuje kolejke generujących nieterminali i na podstawie utrzymywanych struktur oraz generującego nieterminala
wyznacza wszystkie nieterminale, które również są generujące.