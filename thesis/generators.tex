...
\textbf{Call Driven Automaton} jest to automat, dla którego można zdefiniować funkcję $\operatorname{Target}: \Sigma_{call} \rightarrow Q$ i podzielić zbiór
...
\begin{aligned}
    \delta(q_i, c) &= (q_j, s),
    && \text{gdzie } c \in \Sigma_{call},\ q_j = \operatorname{Target}(c),\ s = (i, c) \\
    \delta(q_i^{1}, l) &= (q_i^{2}),
...
jest, aby funkcja $\operatorname{Target}$ spełniała warunek: jeśli $\operatorname{Target}(c_1) \sameModuleRel \operatorname{Target}(c_2)$, to $\operatorname{Target}(c_1) = \operatorname{Target}(c_2)$.
...
Jest zdefiniowana poprzez dostosowanie funkcji $\operatorname{Target}$. Automat $A$ należy do klasy Extended Call Driven
wtedy i tylko wtedy, gdy $\operatorname{Target}(c_1) \sameModuleRel \operatorname{Target}(c_2) \Rightarrow c_1 = c_2$.
