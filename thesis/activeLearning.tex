...
część pracy. W pierwszej kolejności omówiony zostanie klasyczny model aktywnego uczenia automatów DFA oraz jego rozszerzenie
...
\textbf{Model aktywnego uczenia} zakłada istnienie \emph{nauczyciela}, który potrafi odpowiadać na określony zestaw zapytań dotyczących nieznanego
języka docelowego $L$. Algorytm konstruujący automat $H$, nazywany \emph{hipotezą}, który docelowo powinien rozpoznawać język $L$, nazywany jest \emph{uczniem}.
...
\end{itemize}
W odpowiedzi na zapytanie o równoważność nauczyciel może ją potwierdzić, co kończy działanie algorytmu, albo zwrócić \emph{kontrprzykład},
...
Autorzy wprowadzają pojęcie \emph{kontrolnej prawostronnej kongruencji}. Relacja $\simeq$ na słowach nad alfabetem $(\Sigma \cup \underline{\Gamma})$,
...
wykorzystanie systemów SRS dla automatów DVPA. Autor niniejszej pracy jest jej współautorem,
a opisana w dalszych rozdziałach implementacja stanowiła podstawę przedstawionych w niej eksperymentów.

W przypadku języków visibly pushdown istotne jest, że słowa mają dodatkową strukturę wynikającą z dopasowania
...
W publikacji tej wskazano również~\cite[Example 4]{SrsDvpa}, że niektóre naturalne własności języków visibly pushdown
...
Pierwszym rozwiązaniem heurystycznym jest dodanie \emph{wymuszonych przejść}.
Mechanizm ten opisano w podrozdziale~\ref{chap:counterexample} W trakcie implementacji zaobserwowano,
