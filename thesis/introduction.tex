...
czy formalna specyfikacja protokołu TCP~\cite[6.2 The specification of the TCP automaton]{tcp}. Zachowania takich systemów mogą zostać opisane w postaci formalnych modeli automatów, co umożliwia ich analizę oraz
...
który przedstawił w jaki sposób można odtworzyć strukturę automatu na podstawie obserwacji. Zakładając możliwość odpowiadania na dwa rodzaje zapytań:
"Czy słowo $w$ jest akceptowane przez automat?" oraz "Czy dla zadanego automatu $H$, istnieje słowo, które odróżnia go od uczonego systemu?",
...
co umożliwia wykonywanie m.in. takich operacji jak suma, różnica, czy przekrój dwóch automatów w czasie wielomianowym~\cite[2.3 Decision problems]{vpl}.
...
\textbf{Aktywne uczenie się z dostępem do stosu.} W 2022 roku Jan Otop i Jakub Michaliszyn przedstawili algorytm do aktywnego uczenia się automatów DVPA.
...
Praca przedstawia implementację tego algorytmu oraz analizę jego wydajności pod względem liczby wymaganych zapytań dla różnych klas automatów DVPA.
Badany jest również wpływ zastosowania podpowiedzi w postaci reguł przepisywania słów na podstawie pracy \textit{Active Automata Learning with Advice}~\cite{srs},
